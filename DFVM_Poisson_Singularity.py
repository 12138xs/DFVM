...
class PoissonEQuation(object):
    ...
    # Randomly sample bdsize points within the neighborhood of point x
...
class DFVMsolver(object):
    ...
    # Compute the divergence of u = u_theta
    def Nu(self, X):
        u = self.model(X)
    ...
    # Compute the boundary integral for each sample point neighborhood
    def integrate_BD(self, X, x_bd, bd_dir):
        n = len(X)
        integrate_bd = torch.zeros(n, 1).to(self.device)
        # Compute gradient
        Du = self.Nu(x_bd).reshape(n, -1)
        # Multiply gradient by normal vector matrix to get directional derivative
